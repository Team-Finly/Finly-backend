...
public class StockSearchConverter {
    ...
    public StockSearchResDTO toStockSearchResponse(String keyword, Page<Stock> stockPage) {
        List<StockSearchResDTO.Stock> stocks = stockPage.getContent()
                .stream()
                .map(this::toStock)   // 여기서 반환 타입이 StockSearchResponse.Stock 여야 함
                .toList();

        StockSearchResDTO.PageInfo pageInfo = StockSearchResDTO.PageInfo.builder()
        ...
        return StockSearchResDTO.builder()
    ...
    private StockSearchResDTO.Stock toStock(Stock stock) {
        return StockSearchResDTO.Stock.builder()
                .id(stock.getId())
